...
public class VerticleMain extends AbstractVerticle {
    ...
    public void start() throws Exception {
        ...
        server.requestHandler(router).listen(port, handler -> {
            log.info("vertx run prot : [{}] run state : [{}]", port, handler.succeeded());
        });
    }
     ...
     * register controller
     */
    private void registerController(@NotNull Router router, String packagePath) {// "io.javac.vertx.vertxdemo.controller"
        ...
        try {
            ...
            for (Resource resource : resources) {
                String absolutePath = packagePath+"."+resource.getFilename().substring(0,resource.getFilename().lastIndexOf("."));
